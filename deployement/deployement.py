...
class Deployement:
    ...
    def __init__(self) -> None:
        ...
        self.__smoothie: adapters.SmoothieAdapter = adapters.SmoothieAdapter(utility.get_smoothie_vesc_addresses()["smoothie"], calibration_at_init=False)
    ...
    def __init_flask_route(self):
        ...
        self.__app.add_url_rule("/register", methods = ['POST'], view_func=self.__register)
        ...
        self.__app.add_url_rule("/camera_crop_picture", view_func=self.__camera_crop_picture)
        self.__app.add_url_rule( "camera_target_detection", view_func=self.__camera_target_detection)
        self.__app.add_url_rule("/camera_target_move", view_func=self.__camera_target_move)
        self.__app.add_url_rule("/client_config", view_func=self.__client_config)
        self.__app.add_url_rule("/end", view_func=self.__end)

    def __init_socketio(self):
        self.__socketio.on_event('client_config', self.__on_client_config, namespace='/server')
        self.__socketio.on_event('validate_log', self.__on_validate_log, namespace='/server')
        self.__socketio.on_event('run_round_detection', self.__on_run_round_detection, namespace='/server')
        self.__socketio.on_event('run_target_detection', self.__on_run_target_detection, namespace='/server')
        self.__socketio.on_event('run_move_to_target', self.__on_run_move_to_target, namespace='/server')
        self.__socketio.on_event('move_step', self.__on_move_step, namespace='/server')
        self.__socketio.on_event('x_y_dir', self.__on_x_y_dir, namespace='/server')
    ...
    def show_pdf(self, filename):
        if not "path" in send_from_directory.__code__.co_varnames:
            response=make_response(send_from_directory(self.__app.static_folder,filename=f'pdf/{filename}.pdf'))
        else:
            response=make_response(send_from_directory(self.__app.static_folder,path=f'pdf/{filename}.pdf'))
    ...
    def __client_config(self):
        self.__changeConfigValue("CORK_TO_CAMERA_DISTANCE_X",self.__config.CORK_TO_CAMERA_DISTANCE_X+self.__offset_x)
        self.__changeConfigValue("CORK_TO_CAMERA_DISTANCE_Y",self.__config.CORK_TO_CAMERA_DISTANCE_Y+self.__offset_y)
    ...
    def __on_client_config(self, data):
        if data["apply"]:
            ...
            utility.create_directories(f"configFinal{self.__config.ROBOT_SN}","/media/smoothie")
            #python_conf
            ...
            if disk:
                ...
                shutil.chown(config_final_path, "violette", "violette")
            self.__socketio.emit('apply_config', {'apply_done': True}, namespace='/server', broadcast=True)
    ...
    def __on_run_round_detection(self, data):
        if data["run_detection"]:
            ...
            with open('./scene_center.jpg', 'rb') as f:
                image_data = f.read()
            self.__socketio.emit('image', {'image_data': image_data}, namespace='/server', broadcast=True)

    def __on_run_target_detection(self, data):
        if data["run_detection"]:
            res = self.__cameraCalibration.offset_calibration_step_detect(self.__smoothie)
            with open('./target_detection.jpg', 'rb') as f:
                image_data = f.read()
            self.__socketio.emit('image', {'image_data': image_data, "res": res}, namespace='/server', broadcast=True)

    def __on_run_move_to_target(self, data):
        if data["run_move_to_target"]:
            self.__cameraCalibration.offset_calibration_step_move(self.__smoothie)
            self.__socketio.emit('move', {'move': True}, namespace='/server', broadcast=True)
...
def main():
    ...
    try:
        deployement.run(host="0.0.0.0", port="80", debug=True, use_reloader=False)
    finally:
