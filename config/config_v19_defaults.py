...
# multiply factor at the angle of the centroid calculate when the robot is going in the right direction
CENTROID_FACTOR_ORIENTED = 0.585
# multiply factor at the angle of the centroid calculate when the robot isn't going in the right direction
CENTROID_FACTOR_LOST = 0.2

# threshold which represents the maximum deviation of the points in the turns for it to be recognized
...
# anyway, the integral is used for only some hundreds of time
WINDOW = float("inf")
...
MANEUVER_START_DISTANCE = 5000
# when distance to target point is less than specified in the config
USE_SPEED_LIMIT = True
DECREASE_SPEED_TRESHOLD = 5000  # millimeters
# max value and min -value of sum(angles_history), should be positive here, in config
SUM_ANGLES_HISTORY_MAX = 1000
...
# pass by cur points dist between them and prev point is lesser than this, mms
PREV_CUR_POINT_MIN_DIST = 10

# Field creation with main
# allows to generate field by moving forward for a given duration
USE_EMERGENCY_FIELD_GENERATION = False
# mms; side of the area that will be created if emergency field creation is enabled
EMERGENCY_FIELD_SIZE = 45000
...
# number of points that is given to the move to point function in addition to the one that is aimed
...
# Path where the robot goes straight in extraction then reverses without extraction ....
FORWARD_BACKWARD_PATH = False

# This params work only if TRADITIONAL_PATH or BEZIER_PATH are true.
# Adds the path FORWARD_BACKWARD to complete the missing center.
ADD_FORWARD_BACKWARD_TO_END_PATH = True
...
# Allows to determine the number of points put in the bezier turn.
NUMBER_OF_BEZIER_POINT = 11
...
# seconds; how much time cork should move down during plant extraction
EXTRACTION_CORK_DOWN_TIME = 1
...
# is True: adds offsets to control points to make a plant to be at the top half of the undistorted zone
AVOID_CORK_VIEW_OBSCURING = True

# count of full extraction loops called after periphery NN detection (should be >= 1)
EXTRACTIONS_FULL_CYCLES = 2
EXTRACTION_TUNING_MAX_COUNT = 3  # Number of try to get closer to a plant

# mm; if weed is lost after tuning/getting closer - we do 3 shifts for that value (down, left, right) and trying to find it
SEEK_DELTA_DISTANCE = 25
MYOPIA_PATCH = True
...
# freq of sending RPM to vesc for engine in RPM_INCREMENTAL_ENGINE_KEY list.
FREQUENCY_INCREMENTAL_RPM = 0.025
...
# propulsion GPIO stopper PIN number, set to None to disable stopper usage or if no stopper is used for this vesc
VESC_ALLOW_PROPULSION = True
# set to True if GPIO returns 1 if stopper was hit, otherwise set to False
VESC_PROPULSION_STOPPER_PIN = None
# set to True to allow this axis init calibration during vesc adapter instance creation
VESC_PROPULSION_STOP_SIGNAL = False
# Not used in current version of robot
VESC_PROPULSION_CALIBRATE_AT_INIT = False
VESC_PROPULSION_CALIBRATION_RPM = 0  # Not used in current version of robot
# Not used in current version of robot
VESC_PROPULSION_CALIBRATION_MAX_TIME = 2
# this can id will be used if VESC_EXTRACTION_AUTODETECT_CAN_ID is set to False
VESC_PROPULSION_AUTODETECT_CAN_ID = False
VESC_PROPULSION_CAN_ID = None  # parent vesc has can_id=None

# engine 2
# enables extraction vesc initialization and usage
# extraction GPIO stopper PIN number, set to None to disable stopper usage or if no stopper is used for this vesc
VESC_ALLOW_EXTRACTION = True
# VALUE MAY BE DIFFERENT FOR EACH ROBOT # set to True if GPIO returns 1 if stopper was hit, otherwise set to False
VESC_EXTRACTION_STOPPER_PIN = 16
# set to True to allow this axis init calibration during vesc adapter instance creation
VESC_EXTRACTION_STOP_SIGNAL = False
VESC_EXTRACTION_CALIBRATE_AT_INIT = True
VESC_EXTRACTION_CALIBRATION_RPM = -2500  # rpm for init calibration
# seconds; max time needed to reach stopper, calibration will be stopped after this timeout
VESC_EXTRACTION_CALIBRATION_MAX_TIME = 2
VESC_EXTRACTION_CALIBRATION_Z5_FIX_RPM = 2500
# seconds; calibration small movement down time (calibration "Z-5" fix)
VESC_EXTRACTION_CALIBRATION_Z5_FIX_TIME = 0.3
# set to False to use vesc can id from this config, set to True to try detect vesc can id during initialization
VESC_EXTRACTION_AUTODETECT_CAN_ID = False
# this can id will be used if VESC_EXTRACTION_AUTODETECT_CAN_ID is set to False
VESC_EXTRACTION_CAN_ID = 0
...
NO_GPS_TIMEOUT = 5
# Check if gps returned every GPS_CHECK_IN_DEGRADED_MODE navigation cycle.
GPS_CHECK_IN_DEGRADED_MODE = 30
...
# SMOOTHIE SETTINGS
# ======================================================================================================================
# smoothie's ip address for telnet or port for usb serial connector
SMOOTHIE_HOST = "/dev/ttyACM0"
# Deprecated for most scripts that use a function to find it dynamically.
...
# should be always positive, sign will be auto-defined using *_AXIS_CALIBRATION_TO_MAX flag key
...
# smoothie global coordinates to take photos from for forming plants list. Format is (x, y)
CAMERA_POSITIONS = [(X_MAX/2, 0)]
...
# precice detection zone sizes. At each camera position scan only plants inside this zone is added to extraction list
PDZ_DISTANCES = [{"top": 1000, "bot": 1000, "left": 1000, "right": 1000}]
...
X_MOVEMENT_IMAGE_ZONES = [
    {"top": 1000, "bot": 1000, "left": 1000, "right": 1000}]
...
# If this is activated, stops the robot when it no longer has quality 4.
GPS_QUALITY_IGNORE = False
...
# minimum of the slider allowing to configure the second segment of the terrain on the ui.
SLIDER_CREATE_FIELD_MIN = 15
# maximum of the slider allowing to configure the second segment of the terrain on the ui
SLIDER_CREATE_FIELD_MAX = 150
# default value of the slider allowing to configure the second segment of the terrain on the ui
SLIDER_CREATE_FIELD_DEFAULT_VALUE = 25
# step of the slider allowing to configure the second segment of the terrain on the ui
...
# defines how many times robot will try to extract plant or plants group in undist. zone
EXTRACTION_TRIES_PER_PLANT = 3
# should be >= 1; expected value that equal to extraction strategies count so each of them can be applied

# mm; offset size to "remove" corkscrew tube from camera-plant view
AVOID_CORK_VIEW_OBSCURING_DIST_X = 10
...
# will try to move over supposed plant position if plant wasn't detected during approaching
ALLOW_DELTA_SEEKING = True
...
# px; trigger distance for previous option key
FILTER_EXT_PLANTS_TRIGGER_DIST = 25
...
# ======================================================================================================================
NTRIP = True
# FOR CENTIPEDE: Allows you to find the station closest to MAX_DISTANCE_MOUNTPOINT maximum.
FIND_MOUNTPOINT = False
# FOR MOVERTK: Allows us to send our coordinates to the rtk caster
...
NTRIP_PORT = 2101
# Mountpoint of the caster, if SEND_LOCATION_TO_NTRIP=true the program will take the closest.
...
# Allows you to find the station closest to MAX_DISTANCE_MOUNTPOINT maximum if FIND_MOUNTPOINT=True.
MAX_DISTANCE_MOUNTPOINT = 1000
# Id of the rtk frames that will be sent to the gps
...
# SEEDER SETTINGS
# ======================================================================================================================
# how many times to plant seeds at hole after extraction; 0 means disabled
SEEDER_QUANTITY = 0
SEEDER_FILL_DELAY = 0.5  # delay between two M280 commands
# seconds; delay time to allow seeder finish his job (was SEEDER_DELAY earlier)
SEEDER_PLANT_DELAY = 0.3
# mm; if not 0 - will move cork for this value on X axis before using seeder (extraction mode)
SEEDER_EXT_OFFSET_X = 0
SEEDER_EXT_OFFSET_X_F = 20000  # offset force for X axis
# mm; if not 0 - will move cork for this value on Y axis before using seeder (extraction mode)
SEEDER_EXT_OFFSET_Y = 25
...
# MILLING SETTINGS
# ======================================================================================================================
# seconds; delay to let the cork to reach the ground before moving cork aside
MILLING_CORK_GROUND_REACHING_DELAY = 1
...
# seconds of waiting before frame reading; should be positive or zero; set to 0 if thread cleaning is used
BUFF_CLEANING_DELAY = 0
UNDISTORTED_ZONE_RADIUS = 300
# delay in seconds after robot stop and before second scan (M=1)
DELAY_BEFORE_2ND_SCAN = 0.3
WORKING_ZONE_POLY_POINTS = [[1000, 1050], [30, 1080], [45, 755], [40, 300], [
    640, 115], [1000, 70], [1360, 115], [1860, 300], [1955, 755], [1970, 1080]]

# distance between camera and cork on the robot, X axis, relative, mm
CORK_TO_CAMERA_DISTANCE_X = -4
# distance between camera and cork on the robot, Y axis, relative, mm
CORK_TO_CAMERA_DISTANCE_Y = 25
...
# Allows to display the robot guide points on the ui.
DISPLAY_INSTRUCTION_PATH = False
# Number of guide points display on the ui.
DELTA_DISPLAY_INSTRUCTION_PATH = 15
# Point coordinate for test navigation mode, [[lat,long],speed]
