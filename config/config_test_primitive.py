...
# ======================================================================================================================
# 21 #3 # # distance between camera and cork on the robot, X axis, relative, mm
CORK_TO_CAMERA_DISTANCE_X = 2
# 24 #25# distance between camera and cork on the robot, Y axis, relative, mm
CORK_TO_CAMERA_DISTANCE_Y = 28
...
KP = {0.7:0.2*0.55, 0.175:0.2, -0.175:0.10}#0.225
KI = {0.7:0.2*0.91, 0.175:0.0092, -0.175:0.0025}#0.02
...
# max allowed robot's deviation from course im mm-s (threshold)
COURSE_SIDE_DEVIATION_MAX = 50
...
# nav wheels turn value when trying to get back to the course (SHOULD BE POSITIVE VALUE!)
COURSE_ADJ_SMC_VAL = 10
...
# maximum allowable distance between consecutive points (in millimeters)
FILTER_MAX_DIST = 5000
# minimum allowable distance between consecutive points (in millimeters)
FILTER_MIN_DIST = 600
...
# the output of the KP KI is boost if the target is far than this threshold
FAR_TARGET_THRESHOLD = {-0.175: 25000, 0.175: 25000, 0.7: 25000, 0: 25000}
# the output of the KP KI is boost by this muliplier if the target is far than the threshold
FAR_TARGET_GAIN = {-0.175: 1.15, 0.175: 1.15, 0: 1.15, 0.7: 1.15}
# the output of the KP KI is managed if the target is less than this threshold
CLOSE_TARGET_THRESHOLD = {-0.175: 5000, 0.175: 5000, 0: 5000, 0.7: 5000}
# the output of the KP KI is calm by a muliplier if the raw angle is less than the threshold
SMALL_RAW_ANGLE_SQUARE_THRESHOLD = {-0.175: 100, 0.175: 100, 0: 100, 0.7: 100}
# the output of the KP KI is calm by this muliplier if the raw angle is less than the threshold
SMALL_RAW_ANGLE_SQUARE_GAIN = {-0.175: 0.9, 0.175: 0.9, 0: 0.9, 0.7: 1}
# the output of the KP KI is boost by a muliplier if the raw angle is more than the threshold
BIG_RAW_ANGLE_SQUARE_THRESHOLD = {-0.175: 625, 0.175: 625, 0: 625, 0.7: 625}
# the output of the KP KI is boost by this muliplier if the raw angle is more than the threshold
BIG_RAW_ANGLE_SQUARE_GAIN = {-0.175: 1.35, 0.175: 1.35, 0: 1.35, 0.7: 1.35}
#OPEN_LOOP_TF_AMPLITUDES =  [5,       5,           5*12/14, 5*12/17,  5*12/20,    5*12/23] # 2500 Sinus angle range when the servo system is in open loop
# 10000 Sinus angle range when the servo system is in open loop
OPEN_LOOP_TF_AMPLITUDES = [5*9/9,  5*9/13,
                           5*9/24,  5*9/28,   5*9/34,      5*9/40]
# Sinus  sampling range when the servo system is in open loop
OPEN_LOOP_TF_MAX_SAMPLES = [5,        7,
                            10,      15,      20,          40]
# Sinus frequency range when the servo system is in open loop
OPEN_LOOP_TF_FREQUENCIES = [1/5,     1/7,
                            1/10,    1/15,    1/20,        1/40]
...
# length given in mm :  at -2500rpm the deviation get correct after 3 meter path
PURSUIT_LIMIT = 3000


NUMBER_OF_BEZIER_POINT = 10

TRADITIONAL_PATH = False
BEZIER_CORNER_PATH = True
ADD_FORWARD_BACKWARD_PATH_TO_BEZIER_CORNER_PATH = True
JUST_FORWARD_BACKWARD_PATH = False

WHEELS_STRAIGHT_CAHNGE_DIRECTION_OF_TRAVEL = True

NAVIGATION_TEST_MODE = True
...
# pixels; the corkscrew will move so that the plant is at this distance from the upper border of undistorted zone if AVOID_CORK_VIEW_OBSCURING is True
DISTANCE_FROM_UNDIST_BORDER = 100
...
NO_GPS_TIMEOUT = 15
# Check if gps returned every GPS_CHECK_IN_DEGRADED_MODE secondes.
...
EXTRACTION_Z = 0 #10  # 65  # value that passed to smoothie when doing plant extration
...
VIEW_ZONE_POLY_POINTS = [[387, 618], [439, 510], [556, 433], [670, 375], [808, 319], [982, 285], [1143, 279], [1293, 294], [1501, 339], [1635, 395], [1766, 473], [1816, 550], [1867, 637], [1881, 675], [1919, 795], [1942, 926], [1959, 1066], [1964, 1217], [
    1957, 1321], [1949, 1393], [1874, 1425], [1802, 1457], [1692, 1498], [1555, 1537], [1410, 1567], [1219, 1589], [1081, 1590], [944, 1590], [804, 1575], [679, 1552], [569, 1525], [423, 1475], [330, 1431], [277, 1399], [273, 1289], [279, 1131], [297, 976], [343, 780]]
WORKING_ZONE_UNCROPPED_POLY_POINTS = [[1684, 1513], [914, 1448], [929, 1123], [1024, 768], [
    1324, 683], [1684, 638], [2044, 683], [2344, 768], [2439, 1123], [2454, 1448]]
...
#DETECTION
MATRIX_PLANT_ROOT = 1
# All leaft will be number greater than this parameter (2 = one leaft, 3 = two leaft, ...)
MATRIX_PLANT_LEAF = 2
GROUP_THRESHOLD = 3
#EXTRACTION
MATRIX_EXTRACTION = 1
# All different pattern will be number greater than this parameter (2 = pattern one in ExtractionMethods, 3 = pattern two in ExtractionMethods, ...)
MATRIX_EXTRACTION_PATTERN = 2
#ALL
