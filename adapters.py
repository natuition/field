...
class SmoothieAdapter:
    """Provides connection, control, calculations and security control over smoothieboard.

    Make sure to add changes also to SmoothieAdapterStub when editing this class to keep it's stub version up to date.
    """
    ...
    def disconnect(self):
        """An obsolete method for backward compatibility, use close() method or context manager instead.
        """
...
class SmoothieAdapterStub:
    """Simulates SmoothieAdapter class functionality without connection to smoothieboard.

    It is very advised to keep this class up to date when editing SmoothieAdapter."""
    ...
    def halt(self):
        with self.__sync_locker:
            # expected that real smoothie returns this
            # "ok Emergency Stop Requested - reset or M999 required to exit HALT state\r\n"
            return "ok Emergency Stop Requested - reset or M999 required to exit HALT state\r\n"

    def reset(self):
        with self.__sync_locker:
            # TODO check what is real smoothie answer
            return self.RESPONSE_OK
    ...
    def checkendstop(self, axe):
        # with self.__sync_locker:
        raise NotImplementedError("this function stub is not implemented")
    ...
    def get_smoothie_current_coordinates(self, convert_to_mms=True):
        ...
        with self.__sync_locker:
            coordinates = self.get_adapter_current_coordinates()
            if not convert_to_mms:
                for key in coordinates:
                    coordinates[key] = self.mm_to_smoothie(coordinates[key], key)
            return coordinates
    ...
    def mm_to_smoothie(mm_axis_val, axis_label: str):
        ...
        if axis_label not in ["X", "Y", "Z", "A", "B", "C"]:
            raise ValueError("unsupported axis label or wrong type")
        if not SmoothieAdapterStub.__check_arg_types([int, float], mm_axis_val):
            raise TypeError("axis_value should be float or int")
    ...
    def smoothie_to_mm(sm_axis_val, axis_label: str):
        ...
        if axis_label not in ["X", "Y", "Z", "A", "B", "C"]:
            raise ValueError("unsupported axis label or wrong type")
        if not SmoothieAdapterStub.__check_arg_types([int, float], sm_axis_val):
            raise TypeError("axis_value should be float or int")
...
class VescAdapterV3:
    ...
    Make sure to add changes also to VescAdapterV3Stub when editing this class to keep it's stub version up to date.
    ...
    def __init__(self, ser_port, ser_baudrate, alive_freq, check_freq, stopper_check_freq):
        ...
        # TODO better to implement this in main alive control thread
    ...
    def close(self):
        ...
        self.__keep_thread_rpm_alive = False
        cleanup_gpio = False
...
class VescAdapterV3Stub:
    """Provides multiple vesc control simulation without real vesc connection.

    It is very advised to keep this class up to date when editing VescAdapterV3.
    ...
    def __init__(self, ser_port, ser_baudrate, alive_freq, check_freq, stopper_check_freq):
        # TODO remove GPIO usage in case it should be simulated too
    ...
    # TODO add flag "stop engine at stopper hit"
    # TODO will hang until user won't hit stopper manually
    def wait_for_stopper_hit(self, engine_key, timeout=None, stop_engine_if_timeout=True):
        """Blocks caller thread until specified engine stopper hit or timeout time is out (if timeout was set).
