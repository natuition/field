...
from notificationV2 import NotificationClientV2
...
def move_to_point_and_extract(coords_from_to: list,
                              ...
                              notificationV2: NotificationClientV2,
    ...
    while True:
        ...
        if config.GPS_QUALITY_IGNORE and cur_pos[2] != "4":
            ...
            while True:
                ...
                if cur_pos[2] == "4":
                    logger_full.write_and_flush(
                        "The gps has regained quality 4, relaunch of the robot.\n")
                    vesc_engine.start_moving(vesc_engine.PROPULSION_KEY)
                    break
        ...
        if config.CONTINUOUS_INFORMATION_SENDING and not degraded_navigation_mode:
            ...
            notificationV2.set_current_coordinate(cur_pos)
        ...
        lastNtripRestart = navigation.NavigationV3.check_reboot_Ntrip(
            gps_quality, lastNtripRestart, logger_full)
        ...
        if config.CONTINUOUS_INFORMATION_SENDING and vesc_data is not None and "input_voltage" in vesc_data:
            ...
            notificationV2.set_input_voltage(vesc_data["input_voltage"])
...
def main():
    ...
    notificationV2 = NotificationClientV2(time_start)
    data_collector = datacollection.DataCollector(
    ...
    if config.NN_MODELS_COUNT > 1:
        ...
        if config.PRECISE_WRAPPER == 1:
            ...
            if config.CONTINUOUS_INFORMATION_SENDING:
                ...
                notificationV2.set_treated_plant(
                    precise_detector.get_classes_names())
        ...
        with \
            ...
            if config.CONTINUE_PREVIOUS_PATH:
                ...
                if config.CONTINUOUS_INFORMATION_SENDING:
                    link_path = subprocess.check_output(
                        ['readlink', '-f', 'field.txt']).decode("utf-8")
                    field_name = (link_path.split("/")[-1]).split(".")[0]
                    notification.set_field(load_coordinates(
                        config.INPUT_GPS_FIELD_FILE), field_name)
                    notificationV2.set_field(load_coordinates(
                        config.INPUT_GPS_FIELD_FILE), field_name)
                ...
                with open(config.PREVIOUS_PATH_INDEX_FILE, "r") as path_index_file:
                    ...
                    if str_index == "":
                        ...
                        notification.setStatus(SyntheseRobot.HS)
                        exit(1)
                    # TODO check if possible to convert
                ...
                if path_start_index == -1:
                    ...
                    msg = "Path start index " + str(path_start_index) + " is out of path points list range (loaded " + \
                        str(len(path_points)) + \
                        " points) (start index can't be zero as 1rst point is starting point)"
                    print(msg)
                    logger_full.write(msg + "\n")
                    notification.setStatus(SyntheseRobot.HS)
                    exit(1)
            ...
            # load field points and generate new path
                ...
                if config.CONTINUOUS_INFORMATION_SENDING:
                    ...
                    notificationV2.set_field(field_gps_coords, field_name)
