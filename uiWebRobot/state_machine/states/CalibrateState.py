...
class CalibrateState(State):
    ...
    def __init__(self,
        ...
        self.__cork_to_camera_distance = {
            "X": config.CORK_TO_CAMERA_DISTANCE_X, "Y": config.CORK_TO_CAMERA_DISTANCE_Y}
        ...
        self.__coins_in_undistorted_zone = None
        ...
        self.statusOfUIObject = {"currentHTML": "CalibrateDetect.html"}
    ...
    def on_event(self, event):
        if event == Events.CALIBRATION_DETECT:
            ...
            self.socketio.emit(
                'href_to', {"href": "/calibrate"}, namespace='/server', broadcast=True)
            return self
        elif event == Events.CALIBRATION_VALIDATE:
            utilsFunction.changeConfigValue(
                "CORK_TO_CAMERA_DISTANCE_X", self.__cork_to_camera_distance["X"])
            utilsFunction.changeConfigValue(
    ...
    def on_socket_data(self, data):
        if data["type"] == "run_move_to_target":
            # self.cameraCalibration.offset_calibration_step_move_with_distance(
            #    self.smoothie, self.__cork_to_camera_distance["X"], self.__cork_to_camera_distance["Y"])
            self.cameraCalibration.offset_calibration_step_move_with_distance_from_coins(
                self.smoothie, self.__cork_to_camera_distance["X"], self.__cork_to_camera_distance["Y"])
            self.socketio.emit(
                'move', {'move': True}, namespace='/server', broadcast=True)
            return self
        elif data["type"] == "step_axis_xy":
            self.smoothie.custom_move_for(X_F=config.X_F_MAX, Y_F=config.Y_F_MAX, X=data["value"] if data["axis"].upper(
            ) == "X" else 0, Y=data["value"] if data["axis"].upper() == "Y" else 0)
            self.__cork_to_camera_distance[data["axis"].upper(
            )] += data["value"]
            return self
        return self
