...
map.getView().fit([-126356.581417, 5805615.743593, -126276.194864, 5805679.772325], map.getSize());

//var coords1 = ol.proj.transform([-1.134080483359838,46.15724409033286], 'EPSG:4326', 'EPSG:3857')
//var coords2 = ol.proj.transform([-1.133358358667862,46.15764250395524], 'EPSG:4326', 'EPSG:3857')
//map.getView().fit(coords1.concat(coords2), map.getSize());

//Center map on field : 
//middle of 4 points of field [ ((X1+X2+X3+X4)/4)+0.0004 , ((Y1+Y2+Y3+Y4)/4)-0.0002 ]
/*var coordsCenterTest = ol.proj.transform([(-1.1340771716786118+0.0004), (46.157612190152946-0.0002)], 'EPSG:4326', 'EPSG:3857')
var coordsCenter = ol.proj.transform([-1.1337194210138497, 46.15744329750464], 'EPSG:4326', 'EPSG:3857')
map.getView().animate({
    zoom: map.getView().getZoom() - 2,
    center: coordsCenterTest,
    duration: 250
});*/

var socketButton = io.connect('http://' + document.domain + ':' + location.port + '/coords');
var coordAreRecieve = false
socketButton.on('centerOfMap', function(dataServ) {
    if (!coordAreRecieve){
        var coordsCenterTest = ol.proj.transform([(dataServ[1]+0.0012), (dataServ[0]-0.00025)], 'EPSG:4326', 'EPSG:3857')
        map.getView().animate({
            zoom: map.getView().getZoom() - 2.5,
            center: coordsCenterTest,
            duration: 250
        });
        coordAreRecieve = true
    }
});
